...
(function (root) {
  ...
  function parsePalms(rows) {
    var out = { chapter: null, region: null, multi: false, from: null, to: null, members: [], errors: [] };
    var header = -1;
    for (var i = 0; i < rows.length; i++) {
      var c0 = String(rows[i][0] || '').trim();
      if (c0 === 'Chapter:') out.chapter = String(firstVal(rows[i])).trim();
      else if (c0 === 'Region:' || c0 === 'Regions:') out.region = String(firstVal(rows[i])).trim();
      else if (c0 === 'From:') out.from = toISO(firstVal(rows[i]));
      else if (c0 === 'To:') out.to = toISO(firstVal(rows[i]));
      else if (c0 === 'First Name' || (c0 === 'Chapter' && rows[i].indexOf('First Name') > 0)) { header = i; break; }
    }
    if (header < 0) { out.errors.push('Could not find the "First Name" header row. Is this a Summary PALMS Report (Chapter or Region)?'); return out; }
    ...
    out.multi = idx['Chapter'] !== undefined;
    var fcol = idx['First Name'];
    for (var r = header + 1; r < rows.length; r++) {
      var row = rows[r];
      var first = String(row[fcol] || '').trim();
      var c0 = String(row[0] || '').trim();
      if (!first || c0 === 'Total' || first === 'Visitors' || first === 'BNI' || first === 'Total') continue;
      var g = function (k) { return idx[k] === undefined ? 0 : n(row[idx[k]]); };
      out.members.push({
        chapter: out.multi ? String(row[idx['Chapter']] || '').trim() : out.chapter,
